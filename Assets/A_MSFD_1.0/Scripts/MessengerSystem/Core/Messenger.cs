...
namespace MSFD
{
    #region GLexImprovements
    /// <summary>
    /// Messenger improvements by Alex Grechin
    /// </summary>
    public static class MI 
    {

        public static List<Action<string, MessengerMode, int, string[]>> listeners = new List<Action<string, MessengerMode, int, string[]>>();

        public static void AddMessengerEventsListener(Action<string, MessengerMode, int, string[]> onEventCallback)
        {
            listeners.Add(onEventCallback);
        }
        public static void RemoveMessengerEventsListener(Action<string, MessengerMode, int, string[]> onEventCallback)
        {
            listeners.Remove(onEventCallback);
        }
        public static void DisplayBroadcastedMessage(string eventType, MessengerMode messengerMode, int listenersCount, params string[] args)
        {
#if UNITY_EDITOR
            //string mes = "Broadcasted event: " + eventType + " Mode: " + messengerMode;
            //Debug.Log(mes);
            try
            {
                foreach (var x in listeners)
                {
                    x.Invoke(eventType, messengerMode, listenersCount, args);
                }
            }
            catch(Exception exp)
            {
        ...
        public static string FormatArg<T>(T arg)
        {
            string str;
            UnityEngine.Object obj = arg as UnityEngine.Object;
            if (obj != null)
            {
                str = typeof(T).Name + ": " + obj.name + " (" + obj.GetInstanceID() + ")";
            }
            else
            {
                str = typeof(T).Name + ": " + arg.ToString();
            }
            return str;
        }

        public static void DisplayErrorMessageWhenBroadcast(string callbackTarget, string callbackMethod, Exception exp)
        {
            UnityEngine.Debug.LogError(FormCorrectErrorMessageWhenBroadcast(callbackTarget, callbackMethod, exp));
        }
        static string FormCorrectErrorMessageWhenBroadcast(string callbackTarget, string callbackMethod, Exception exp)
        {
            string mainMessage = exp.StackTrace.Substring(0, exp.StackTrace.IndexOf("\n"));
            return "Messenger error: " + exp.Message + mainMessage + "\n" +
            "\nError starts from class: " + callbackTarget + " in method " + callbackMethod +
                "\n\n" + exp;
        }
    }
    #endregion
    ...
    static internal class MessengerInternal
    {
        ...
        static public T[] GetInvocationList<T>(string eventType) where T : Delegate
        {
            Delegate d;
            if (eventTable.TryGetValue(eventType, out d))
            {
                if (d != null)
                {
                    ...
                    /*List<T> invocationList = new List<T>();
                    foreach(Delegate x in d.GetInvocationList())
                    {
                        if (x is T)
                        {
                            invocationList.Add((T)x);
                        }
                        else
                        {
                            UnityEngine.Debug.LogError($"Error in converting type in {x.Target} {x.Method}");
                        }
                    }
        ...
        {
            //MI.DisplayBroadcastedMessage(eventType, mode);
    ...
    {
        #region GLexImprovements
        #region Disposable Subscribe
        public static IDisposable Subscribe(string eventType, Action handler)
        {
            AddListener(eventType, handler);
            return Disposable.Create(() => RemoveListener(eventType, handler));
        }
        ...
        {
#if UNITY_EDITOR
            var il = MessengerInternal.GetInvocationList<Action>(eventType);
            MI.DisplayBroadcastedMessage(eventType, mode, il?.Length ?? 0);
#endif

            MessengerInternal.OnBroadcasting(eventType, mode);
            var invocationList = MessengerInternal.GetInvocationList<Action>(eventType);

            if (mode == MessengerMode.DONT_REQUIRE_LISTENER && invocationList == null)
            {
                return;
            }
            foreach (var callback in invocationList)
            {
                ...
                {
                    MI.DisplayErrorMessageWhenBroadcast(callback.Target.ToString(), callback.Method.ToString(), exp);
                }
            }         
        }
    ...
    {
        #region Disposable Subscribe
        public static IDisposable Subscribe(string eventType, Action<T> handler)
        {
            AddListener(eventType, handler);
            return Disposable.Create(() => RemoveListener(eventType, handler));
        }
        ...
        {
#if UNITY_EDITOR
            var il = MessengerInternal.GetInvocationList<Action<T>>(eventType);
            MI.DisplayBroadcastedMessage(eventType, mode, il?.Length ?? 0, MI.FormatArg(arg1));
#endif
            MessengerInternal.OnBroadcasting(eventType, mode);
    ...
    {
        #region Disposable Subscribe
        public static IDisposable Subscribe(string eventType, Action<T, U> handler)
        {
            AddListener(eventType, handler);
            return Disposable.Create(() => RemoveListener(eventType, handler));
        }
        ...
        {
#if UNITY_EDITOR
            var il = MessengerInternal.GetInvocationList<Action<T, U>>(eventType);
            MI.DisplayBroadcastedMessage(eventType, mode, il?.Length ?? 0, MI.FormatArg(arg1), MI.FormatArg(arg2));
#endif
            MessengerInternal.OnBroadcasting(eventType, mode);
    ...
    {
        #region Disposable Subscribe
        public static IDisposable Subscribe(string eventType, Action<T, U, V> handler)
        {
            AddListener(eventType, handler);
            return Disposable.Create(() => RemoveListener(eventType, handler));
        }
        ...
        {
#if UNITY_EDITOR
            var il = MessengerInternal.GetInvocationList<Action<T, U, V>>(eventType);
            MI.DisplayBroadcastedMessage(eventType, mode, il?.Length ?? 0, MI.FormatArg(arg1), MI.FormatArg(arg2), MI.FormatArg(arg3));
#endif
            MessengerInternal.OnBroadcasting(eventType, mode);
