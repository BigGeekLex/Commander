            ...
            public Vector3 UpdateShake()
            {
                ...
                if (fadeInDuration > 0 && sustain)
                {
                    if (currentFadeTime < 1)
                        currentFadeTime += UnityEngine.Time.deltaTime / fadeInDuration;
                    else if (fadeOutDuration > 0)
                        sustain = false;
                }

                if (!sustain)
                    currentFadeTime -= UnityEngine.Time.deltaTime / fadeOutDuration;

                if (sustain)
                    tick += UnityEngine.Time.deltaTime * Roughness * roughMod;
                else
                    tick += UnityEngine.Time.deltaTime * Roughness * roughMod * currentFadeTime;
